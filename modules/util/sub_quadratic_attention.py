...
def _summarize_chunk(
    ...
    max_score = max_score.detach()  # Detach to avoid gradient computation
    attn_weights -= max_score  # Shift for numerical stability

    if mask is not None:
        attn_weights += mask  # Apply the mask

    # Avoid using `out=` for compatibility with autograd
    attn_weights = torch.exp(attn_weights)

    # Cast weights to the same dtype as the value tensor for consistency
    exp_weights = attn_weights.to(value.dtype)

    # Compute the attention output
    exp_values = torch.bmm(exp_weights, value)

    # Squeeze max_score to match dimensions
    max_score = max_score.squeeze(-1)

    # Return the computed values, weights sum, and max score
...
def efficient_dot_product_attention(
    ...
    if query_chunk_size is None and kv_chunk_size is None:
        stats = torch.cuda.memory_stats()
        mem_active = stats['active_bytes.all.current']
        mem_reserved = stats['reserved_bytes.all.current']
        mem_free_cuda, _ = torch.cuda.mem_get_info()
